...
def parse_source_save_path(save_path):
    ...
    return os.path.normpath(source_save_path).replace('\\', '/')
...
def preprocess_video(video_path, save_path, face_detector, face_predictor):
    ...
    frame_idxs = np.linspace(0, frame_count_video - 1, NUM_FRAMES, endpoint=True, dtype=np.int64)
    # process each frame
    for cnt_frame in range(frame_count_video):
        ...
        print(os.path.normpath(f"{save_path}/frame_{cnt_frame}").replace('\\', '/'))
        print(os.path.normpath(f"{source_save_path}/frame_{cnt_frame}").replace('\\', '/'))
        video_dict['landmark'] = landmarks.tolist()
        video_dict['source_path'] = os.path.normpath(f"{source_save_path}/frame_{cnt_frame}").replace('\\', '/')
        video_dict['label'] = label
        IMG_META_DICT[os.path.normpath(f"{save_path}/frame_{cnt_frame}").replace('\\', '/')] = video_dict
        # save one frame
    ...
    for dataset in DATASETS:
        for comp in COMPRESSION:
            ...
            for i in tqdm(range(n_sample)):
                save_path_per_video = os.path.normpath(
                    movies_path_list[i].replace(VIDEO_PATH, SAVE_IMGS_PATH)
                    .replace('.mp4', '')
                    .replace("/videos", "/frames")
                ).replace('\\', '/')
                print(save_path_per_video)
                preprocess_video(
